...
def get_city_name(content):
    """Extract city name from h1"""
    match = re.search(r'<span className="text-yellow-400">([^<]+)</span>', content)
    if match:
        return match.group(1)
    return None

def add_trust_section(page_path):
    """Add trust/why choose section"""
    content = page_path.read_text(encoding='utf-8')

    city_name = get_city_name(content)
    if not city_name:
        return False

    # Check if already has trust section
    if "Waarom kiezen voor" in content:
        return False
...
'''
    ...
    # Insert trust section after Process Section (before SEO Content Section)
    pattern = r'(        </section>\n\n        \{/\* Services Section \*/\}\n        \{/\* SEO Content Section \*/\})'
...
for page_path in computerhulp_pages:
    ...
    if add_trust_section(page_path):
        print(f"Added: {city_slug}")
        added += 1

print(f"\nAdded trust section to {added}/{len(computerhulp_pages)} pages")
